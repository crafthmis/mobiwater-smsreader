...
public class DBHelperOld extends SQLiteOpenHelper {
        ...
        try {
            ...
            if (cursor != null) {
                do {
                    ...
                    //if ((l1 > l2) && (!meterReading.equalsIgnoreCase("") || meterReading != null) && !(sms.toLowerCase().contains("sms report")|| sms.toLowerCase().contains("tel")|| sms.toLowerCase().contains("rtu power")|| sms.toLowerCase().contains("sms format error")|| sms.toLowerCase().contains("ac power")|| sms.toLowerCase().contains("humidity")|| sms.toLowerCase().contains("host arm")|| sms.toLowerCase().contains("server")|| sms.toLowerCase().contains("gprs always online")|| sms.toLowerCase().contains("armed")|| sms.toLowerCase().contains("disarmed"))==true) {
                    if ((l1 > l2) && (!meterReading.equalsIgnoreCase("") || meterReading != null) && !(sms.toLowerCase().contains("sms report")|| sms.toLowerCase().contains("tel")|| sms.toLowerCase().contains("rtu power")|| sms.toLowerCase().contains("sms format error")|| sms.toLowerCase().contains("ac power") || sms.toLowerCase().contains("host arm")|| sms.toLowerCase().contains("server")|| sms.toLowerCase().contains("gprs always online")|| sms.toLowerCase().contains("armed")|| sms.toLowerCase().contains("disarmed"))==true) {
    ...
    public LineData drawWeekGraph(String uId, int dateInterval) {
        ...
        int j = 0;
        try {
            String[] weekRange = getWeekRange(dateInterval).split("-");
            cursor = db.query(TABLE_TANK, new String[]{TANK_LEVEL}, "tnk_uid = " + uId + " AND " + TANK_TIME + " BETWEEN " + weekRange[0] + " AND " + weekRange[1], null, null, null, "tank_time asc");
            cursor.moveToFirst();
            if (!cursor.isAfterLast()) {
                do {
                    entries.add(new Entry((float) j, cursor.getFloat(0)));
    ...
    public HashMap<Integer, String> LineDataXWeekLabels(String uId, int dateInterval) {
        db = getReadableDatabase();
        Cursor cursor = null;
        HashMap<Integer, String> hashMap = new HashMap();
        int j = 0;
        try {
            String[] weekRange = getWeekRange(dateInterval).split("-");
            cursor = db.query(TABLE_TANK, new String[]{TANK_TIME}, "tnk_uid = " + uId + " AND " + TANK_TIME + " BETWEEN " + weekRange[0] + " AND " + weekRange[1], null, null, null, "tank_time asc");
            cursor.moveToFirst();
    ...
    public static  String getWeekRange (int interval) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTimeZone(TimeZone.getTimeZone("GMT+3"));
        int dayOfMonth = calendar.get(DAY_OF_MONTH);
        calendar.set(Calendar.DAY_OF_MONTH, dayOfMonth+interval);
